...
//-----------------------------------------------------------------------------
{
  ...
  //New Variables
  bool validCube = true;
  bool edgeMarkCube = true;
  float dpts[8], epts[8];
  int countCubes_inDensityRange=0, countCubes_withEdgeMarkers=0;	
  //-------------------------------
...
//_____________________________________________________________________________
//New function: MSD:20130107
void MarchingCubes::getEightDensityPoints(float zc[], Matrix3D<float>& dataPointObj, Point& ci){
	
	zc[0] = dataPointObj.get(ci.x(), ci.y(), ci.z());       //v_left_down    (front)
	zc[1] = dataPointObj.get(ci.x()+1, ci.y(), ci.z());		//v_right_down
	zc[2] = dataPointObj.get(ci.x()+1, ci.y()+1, ci.z());   //v_right_up
	zc[3] = dataPointObj.get(ci.x(), ci.y()+1, ci.z());      //v_left_up
	
	zc[4] = dataPointObj.get(ci.x(), ci.y(), ci.z()+1);     //v_left_down     (back)
	zc[5] = dataPointObj.get(ci.x()+1, ci.y(), ci.z()+1);   //v_right_down
	zc[6] = dataPointObj.get(ci.x()+1, ci.y()+1, ci.z()+1); //v_right_up
	zc[7] = dataPointObj.get(ci.x(), ci.y()+1, ci.z()+1);   //v_left_up
}
...
void MarchingCubes::getSixNeighborVoxels(float nv[], Matrix3D<float>* dataPointObj, Point& ci){
	
	nv[0] = dataPointObj->get(ci.x()-1, ci.y(), ci.z());     //v_left
	nv[1] = dataPointObj->get(ci.x()+1, ci.y(), ci.z());	  //v_right
	nv[2] = dataPointObj->get(ci.x(), ci.y()-1, ci.z());     //v_down
	nv[3]= dataPointObj->get(ci.x(), ci.y()+1, ci.z());      //v_up
	nv[4] = dataPointObj->get(ci.x(), ci.y(), ci.z()-1);     //v_back
	nv[5] = dataPointObj->get(ci.x(), ci.y(), ci.z()+1);     //v_front
}
//_____________________________________________________________________________
...
void MarchingCubes::build_ext_triangles(MarchingCube_header& mcHeader, Matrix3D<float>* origVolObj){
	...
	vector<Vector> nlv;								//normals
	float nv[6];								    //density from six neighbor voxels
	float gx, gy, gz;							    //gradient normals;
	int v1_idx, v2_idx, v3_idx;
	int p0_x=0, p0_y=0, p0_z=0, p1_x=0, p1_y=0, p1_z=0, p2_x=0, p2_y=0, p2_z=0;
	
	ptv.resize(3);
	nlv.resize(3);
	
	for (int i = 0; i < _ntrigs; i++ )
	{
		v1_idx = _triangles[i].v1 ;
		v2_idx = _triangles[i].v2 ;
		...
		if(origVolObj==NULL){
			...
			nlv[0]=Vector(_vertices[v1_idx].nx*-1, _vertices[v1_idx].ny*-1, _vertices[v1_idx].nz*-1);
			nlv[2]=Vector(_vertices[v2_idx].nx*-1, _vertices[v2_idx].ny*-1, _vertices[v2_idx].nz*-1);
			nlv[1]=Vector(_vertices[v3_idx].nx*-1, _vertices[v3_idx].ny*-1, _vertices[v3_idx].nz*-1);
			
		}else {
			//rounding off
			/*
			p0_x = _vertices[v1_idx].x;   p0_y = _vertices[v1_idx].y;   p0_z = _vertices[v1_idx].z;
			p1_x = _vertices[v2_idx].x;   p1_y = _vertices[v2_idx].y;   p1_z = _vertices[v2_idx].z;
			p2_x = _vertices[v3_idx].x;   p2_y = _vertices[v3_idx].y;   p2_z = _vertices[v3_idx].z;
			*/
			p0_x = _vertices[v1_idx].x+0.5;   p0_y = _vertices[v1_idx].y+0.5;   p0_z = _vertices[v1_idx].z+0.5;
			p1_x = _vertices[v2_idx].x+0.5;   p1_y = _vertices[v2_idx].y+0.5;   p1_z = _vertices[v2_idx].z+0.5;
			p2_x = _vertices[v3_idx].x+0.5;   p2_y = _vertices[v3_idx].y+0.5;   p2_z = _vertices[v3_idx].z+0.5;
			
			Point p[3] = {Point(p0_x, p0_y, p0_z), Point(p2_x, p2_y, p2_z), Point(p1_x, p1_y, p1_z) };
			...
			for (int j=0; j<3; j++) {
				...
				/*
				if(i>0 && i<=10){
					printf("normal on a face:[%f,%f,%f]\n",nlv[j].x(),nlv[j].y(), nlv[j].z());
				}*/
			}
		}
...
//-----------------------------------------------------------------------------
{
  ...
  PlyProperty face_props[]  = { /* list of property information for a PlyFace */
    {(char*)"vertex_indices", Int32, Int32, offsetof( PlyFace,verts ),
  ...
  describe_element_ply ( ply, (char*)"vertex", _nverts );
  ...
  describe_element_ply ( ply, (char*)"face", _ntrigs );
